...
class Trainer:
    ...
    def forward_and_loss(
        ...
        if epoch % 100 == 0:
            if dataset_mode == 'val':
                mask_labels = batch_dict['val_mask_matrix'][:,0]
                nll_score = self.nll(output[0],mask_labels,ground_truth_tensors[0])
                print(f"val set nll score for {epoch} epochs is {nll_score.item():.2f}")
            elif dataset_mode == 'test':
                mask_labels = batch_dict['test_mask_matrix'][:,0]
                nll_score = self.nll(output[0],mask_labels,ground_truth_tensors[0])
                print(f"test set nll score for {epoch} epochs is {nll_score.item():.2f}")

    def nll(self,output,mask_labels,ground_truth_tensors):
        ground_truth_tensors = torch.argmax(ground_truth_tensors,dim=1)
        nll = torch.nn.NLLLoss(reduction='none')
        output_2 = torch.nn.functional.log_softmax(output,dim=1)
        nll_score = torch.sum(nll.forward(output_2,ground_truth_tensors) * mask_labels)/torch.sum(mask_labels)
        return nll_score
